...
#         ligacao_vertices = arestas[vertice]
#         for j, dict in enumerate(ligacao_vertices):
#             (vertice2, peso) = dict
#             if vertice2 != vertices_visitados[-1]:
#                 if i + 1 < len(vertices_visitados) and vertice2 == vertices_visitados[i + 1]:
#                     arestas[vertice][j] = (vertice2, peso - 1)
#                     for u, dict2 in enumerate(arestas[vertice2]):
#                         (vertice3, peso2) = dict2
#                         if vertice3 == vertice:
#                             arestas[vertice2][u] = (vertice3, peso - 1)
...
# def segunda_fase_iniciar(vertices_primeira_fase):
#     if config.dados["fases"]["atual"] > 2:
#         escreve_introducao_final_fase(texto_introducao_fase)
#         config.TELA.fill(config.BACKGROUND_JOGO)
    
#     update_grafo(vertices_primeira_fase)
