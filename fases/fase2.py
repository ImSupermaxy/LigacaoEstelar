...
def update_grafo(vertices_visitados, end=False):
    ...
    if not config.OPEN_FASE_TWO:  
        ...
        if end:
            config.OPEN_FASE_TWO = True


def desenhar_grafo():
    config.TELA.fill(config.BACKGROUND_JOGO)
    main.desenhar_textos(["FASE 2"], config.ROXO2, 25, config.LARGURA - 140, False, config.FONTE_PESO)
...
def segunda_fase_iniciar(resetar=True):
    ...
    if 2 not in config.FASES_CONCLUIDAS and resetar and not config.SkipHistoria:
    ...
    while tmpI <= 2:
        ...
        update_grafo(visited_nodes, not (tmpI + 1) <= 2)
    ...
    atualiza_dados_fase(2, visited_nodes, soma_arestas)
